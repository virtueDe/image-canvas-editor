import type { EditorState, ImageResource, SerializableEditorState } from './types';
import { loadImageFromDataUrl } from './utils';

const DRAFT_KEY = 'image-canvas-editor:draft';

const toSerializable = (state: EditorState): SerializableEditorState => ({
  image: state.image
    ? {
        width: state.image.width,
        height: state.image.height,
        name: state.image.name,
        dataUrl: state.image.dataUrl,
      }
    : null,
  cropRect: state.cropRect,
  adjustments: state.adjustments,
  transform: state.transform,
  activePreset: state.activePreset,
});

export const saveDraft = (state: EditorState): void => {
  localStorage.setItem(DRAFT_KEY, JSON.stringify(toSerializable(state)));
};

const hydrateImage = async (
  rawImage: SerializableEditorState['image'],
): Promise<ImageResource | null> => {
  if (!rawImage) {
    return null;
  }

  const element = await loadImageFromDataUrl(rawImage.dataUrl);

  return {
    ...rawImage,
    element,
  };
};

export const restoreDraft = async (): Promise<SerializableEditorState & { image: ImageResource | null }> => {
  const rawValue = localStorage.getItem(DRAFT_KEY);

  if (!rawValue) {
    throw new Error('没有找到可恢复的草稿');
  }

  const parsed = JSON.parse(rawValue) as SerializableEditorState;

  return {
    ...parsed,
    image: await hydrateImage(parsed.image),
  };
};
