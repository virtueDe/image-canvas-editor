...
const toSerializable = (state: EditorState): SerializableEditorState => {
  ...
  return {
    ...
    brushToolState: normalizeBrushToolState(state.brushToolState, state.brushStrokes ?? []),
...
export const createLocalDraftStore = (storageKey = DRAFT_STORAGE_KEY): DraftStore => ({
  ...
  async restore() {
    ...
    return {
      ...
      activeTool: parsed.activeTool ?? 'navigate',
      textOverlay: normalizedTextState.textOverlay,
      texts: normalizedTextState.texts.map((text) => ({ ...text })),
      activeTextId: normalizedTextState.activeTextId,
      textToolState: cloneTextToolState(normalizedTextState.textToolState),
      brush: normalizeBrushSettings(parsed.brush ?? createDefaultBrushSettings()),
      brushStrokes: cloneBrushStrokes(parsed.brushStrokes ?? []),
      brushToolState: cloneBrushToolState(
        normalizeBrushToolState(parsed.brushToolState ?? createIdleBrushToolState(), parsed.brushStrokes ?? []),
      ),
